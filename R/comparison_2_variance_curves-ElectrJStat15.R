...
function3 <- function(x1, y1, x2, y2, B = 100, bandwidths = "cv", sigma.w = 1){
  ...
  h.f1 <- bw.ucv(x1, lower = 0.05, upper = 0.50)
  h.f2 <- bw.ucv(x2, lower = 0.05, upper = 0.50)
  ...
  KS.1 <- sqrt(n1) * ks.test(eps01.hat, eps1.hat)$statistic + sqrt(n2) * ks.test(eps02.hat, eps2.hat)$statistic
  KS.2 <- sqrt(n) * ks.test(c(eps01.hat, eps02.hat), c(eps1.hat, eps2.hat))$statistic
  ...
  eps1.hat <- (eps1.hat - mean(eps1.hat)) / sd(eps1.hat)
  eps2.hat <- (eps2.hat - mean(eps2.hat)) / sd(eps2.hat)

  # Matrix A

  a1 <- -(sum((eps1.hat %*% t(eps1.hat)) * D2Iw(outer(eps1.hat,eps1.hat,"-"),sigma.w))-eps1.hat%*%eps1.hat)/(4*n1*(n1-1))
  a2 <- -(sum((eps2.hat %*% t(eps2.hat)) * D2Iw(outer(eps2.hat,eps2.hat,"-"),sigma.w))-eps2.hat%*%eps2.hat)/(4*n2*(n2-1))
  a.matrix <- diag(c(a1, a2))

  # Matrix F
  h.feps1 <- bw.ucv(eps1.hat, lower = 0.10, upper = 1.00)
  h.feps2 <- bw.ucv(eps2.hat, lower = 0.10, upper = 1.00)
  ...
  pvalue.Tn1.asym  <- mean(Tn1 < beta[1] * rchisq(10 ^ 6, 1) + beta[2] * rchisq(10 ^ 6, 1))
  pvalue.CM.1.asym <- mean(CM.1 < tseta[1] * rchisq(10 ^ 6, 1) + tseta[2] * rchisq(10 ^ 6, 1))
  ...
  #	print(c("ib",ib))
      ...
      sigma1x1.hat.boot <- sqrt(abs(nadarayawatson(n1, x1, y1.boot ^ 2, n1, x1, h.sigma1) - nadarayawatson(n1, x1, y1.boot, n1, x1, h.sigma1) ^ 2))
      sigma1x2.hat.boot <- sqrt(abs(nadarayawatson(n1, x1, y1.boot ^ 2, n2, x2, h.sigma1) - nadarayawatson(n1, x1, y1.boot, n2, x2, h.sigma1) ^ 2))
      sigma2x1.hat.boot <- sqrt(abs(nadarayawatson(n2,x2, y2.boot ^ 2, n1, x1, h.sigma2) - nadarayawatson(n2, x2, y2.boot, n1, x1, h.sigma2) ^ 2))
      sigma2x2.hat.boot <- sqrt(abs(nadarayawatson(n2, x2, y2.boot ^ 2, n2, x2, h.sigma2) - nadarayawatson(n2, x2, y2.boot, n2, x2, h.sigma2) ^ 2))
      ...
      Tn1.boot[ib]  <- Tn1.function(n1, n2, n, eps1.hat.boot, eps01.hat.boot, eps2.hat.boot, eps02.hat.boot, sigma.w)

      Tn2.boot[ib]  <- Tn2.function(n1, n2, n, eps1.hat.boot, eps01.hat.boot, eps2.hat.boot, eps02.hat.boot, sigma.w)

      KS.1.boot[ib] <- sqrt(n1)*ks.test(eps01.hat.boot, eps1.hat.boot)$statistic + sqrt(n2) * ks.test(eps02.hat.boot, eps2.hat.boot)$statistic

      KS.2.boot[ib] <- sqrt(n) * ks.test(c(eps01.hat.boot, eps02.hat.boot), c(eps1.hat.boot, eps2.hat.boot))$statistic

      CM.1.boot[ib] <- cramer.vonmises(n1, eps01.hat.boot, n1, eps1.hat.boot) + cramer.vonmises(n2, eps02.hat.boot, n2, eps2.hat.boot)

      CM.2.boot[ib] <- cramer.vonmises(n1 + n2, c(eps01.hat.boot, eps02.hat.boot), n1 + n2, c(eps1.hat.boot, eps2.hat.boot))
