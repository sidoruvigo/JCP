#' @title function2
#' @export
function2 <- function(x1, y1, x2, y2, B = 100, bandwidths = "cv", sigma.w = 1) {
  ...
  KS.1 <- sqrt(n1) * stats::ks.test(eps01.hat, eps1.hat)$statistic + sqrt(n2) * ks.test(eps02.hat, eps2.hat)$statistic
  ...
  # Test statistics
    ...
    KS.1.boot[ib] <- sqrt(n1) * ks.test(eps01.hat.boot, eps1.hat.boot)$statistic +
      sqrt(n2) * ks.test(eps02.hat.boot, eps2.hat.boot)$statistic
    KS.2.boot[ib] <- sqrt(n) * ks.test(c(eps01.hat.boot, eps02.hat.boot),
